...
int main(void)
{
	...
	// The thing Phase 3 is judged on, and the meshes for it. Both happen once, before
	// the loop: chunk streaming is Phase 5, so nothing here is meshed on the fly.
	...
	Player player;
	playerInit(&player, (float)spawn_x + 0.5f,
	           (float)handbuiltHeight(spawn_x, spawn_z), (float)spawn_z + 0.5f,
	...
	worldReportDraw(refused, built, mesh_refused, selftest, &stress, &dig);
