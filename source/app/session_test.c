...
static void testSinglePlayerQuitToTitleLeavesTheTableJoinable(void)
{
	...
	// Forty-four core rows since v1.9.0 added the chest on top of v1.8.15's four cooked meats
	// and the furnace, v1.8.14's four raw meats, v1.8.12's six ores, v1.8.10's torch, v1.8.8's
	// twelve per-biome timber and flora rows, v1.8.3 Phase 3's snow, ice, cactus, dead bush and
	// fern, and tasks 17/19's water and tall grass, plus the one dynamic row this world
	// registered. (The measured symptom quoted above was taken when there were eight core
	// rows; the +1/+0 shape of it is what matters, not the base.)
	CHECK(registryCount() == 45);
...
static void testEveryExitFromAWorldLandsInTheSameState(void)
{
	...
	//
	// 34 -> 38 on 2026-09-03: v1.8.14 "Animals" appends four raw meat rows, ids 34..37. Found
	// the same way the entry above was, and worth saying because the lesson has now repeated
	...
	//
	// 43 -> 44 on 2026-09-05: v1.9.0 "Storage" appends the chest, id 43. Sixth repetition of the
	// identical blind spot — `cold.count`, not `registryCount()`, so nothing that greps for the
	// latter finds it, and this file stayed clean in `git diff` right up until the full-suite
	// run surfaced it (alongside registryCount()'s own five pins in this file, and the
	// boundary/count pins in inventory_test.c). Same instrument, same result, sixth time.
	CHECK(cold.count == 44 && !cold.frozen);
