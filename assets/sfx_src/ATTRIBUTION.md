...
## Per-material footstep/break/place — v1.8.19, no download involved

Twelve sounds for "footsteps that know what you are walking on, block breaking and placing
per material" (STONE, WOOD, DIRT, GRASS). Nine are procedural, same rule as the six above —
generated by `tools/make_sfx_synth.py`, no licence to trace. The other three are NOT new
downloads: they reuse the three Kenney recordings already listed above, decoded again under
a material-specific output name because that recording already is the right material.

| Game sound | Source file | Generator / pack | Licence |
|---|---|---|---|
| `romfs/sfx/footstep_stone.bsnd` | `footstep_stone_synth.wav` | `tools/make_sfx_synth.py::make_footstep_stone` | none — procedural |
| `romfs/sfx/footstep_wood.bsnd` | `footstep_wood_000.ogg` | Kenney — Impact Sounds (reused, see above) | CC0 |
| `romfs/sfx/footstep_dirt.bsnd` | `footstep_dirt_synth.wav` | `tools/make_sfx_synth.py::make_footstep_dirt` | none — procedural |
| `romfs/sfx/footstep_grass.bsnd` | `footstep_grass_synth.wav` | `tools/make_sfx_synth.py::make_footstep_grass` | none — procedural |
| `romfs/sfx/break_stone.bsnd` | `impactMining_000.ogg` | Kenney — Impact Sounds (reused, see above) | CC0 |
| `romfs/sfx/break_wood.bsnd` | `break_wood_synth.wav` | `tools/make_sfx_synth.py::make_break_wood` | none — procedural |
| `romfs/sfx/break_dirt.bsnd` | `break_dirt_synth.wav` | `tools/make_sfx_synth.py::make_break_dirt` | none — procedural |
| `romfs/sfx/break_grass.bsnd` | `break_grass_synth.wav` | `tools/make_sfx_synth.py::make_break_grass` | none — procedural |
| `romfs/sfx/place_stone.bsnd` | `place_stone_synth.wav` | `tools/make_sfx_synth.py::make_place_stone` | none — procedural |
| `romfs/sfx/place_wood.bsnd` | `impactPlank_medium_000.ogg` | Kenney — Impact Sounds (reused, see above) | CC0 |
| `romfs/sfx/place_dirt.bsnd` | `place_dirt_synth.wav` | `tools/make_sfx_synth.py::make_place_dirt` | none — procedural |
| `romfs/sfx/place_grass.bsnd` | `place_grass_synth.wav` | `tools/make_sfx_synth.py::make_place_grass` | none — procedural |

The three reused rows carry no new provenance section of their own: they are the same
bytes, from the same Kenney pack, already verified by SHA-256 in the Provenance section
above — this table just records which game sound each one now also feeds.
