...
`TitleInput { keys_down, keys_held, touch_down, touch_x, touch_y }`; screens MAIN (4 stacked buttons), WORLD_SELECT (`drawWorldSelect` `:444-494`, 4 visible 32-px rows + NEW WORLD + BACK — **[2026-09-07] `drawWorldSelect()` no longer exists** (`grep -rn drawWorldSelect source/` returns nothing); the job now belongs to `drawPlayTab()` at `source/scene/title.c:659`, reached through the new tab bar's PLAY category via `drawBar()` at `title.c:1697`, which calls it at `title.c:1787`), OPTIONS_GENERAL (`drawOptionsGeneral` `:565-641`, `GEN_ITEM_COUNT 8`, three 22-px buttons — `OPT_BTN_H`'s own comment at `:120-123` says the right fix is "CONTROLS/UPDATE/BACK on their own screen, or a scrolling list"), BINDINGS, MULTIPLAYER (dense text rows), UPDATE, VERSION_HISTORY. Helpers `uiButton`, `settingRowStepper`, `settingRowToggle`. Rename/delete from v1.9.0 6.3 are not yet in `title.c` (0 hits) — an in-flight lane is adding them. **[2026-09-07] That lane landed.** `grep -c "worldlistRenameAt\|worldlistDeleteAt" source/scene/title.c` returns **8**, so both are wired through the title screen; the "0 hits" reading above is a snapshot from before v1.9.0 shipped, not the current state.
...
| S5 | `main.c:5870-5871` | `if (!paused && !bar_open && (down & KEY_L))` and the R line — **[2026-09-07] VOIDED, nothing was written.** `grep -n "KEY_L\|KEY_R" source/main.c` returns no code hits (only a comment, at `main.c:5963-5967`, that already records this same finding): v1.9.0 had already moved L/R off live render-distance stepping and onto hotbar cycling, so by the time v1.9.1 started there were no raw `KEY_L`/`KEY_R` render-distance lines left at `main.c:5870-5871` or anywhere else in the file to gate. The plan is preserved above; reality is that this seam had nothing to do. |
